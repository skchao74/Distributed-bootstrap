...
def ci_hd_lm_cv(y, X, inv_cov, k, B, beta_s, s, t):
...
for i in np.arange(len(ds)):
    ...
    for ii in np.arange(len(ss)):
        ...
        for j in np.arange(len(ks)):
            ...
            k = ks[j]
            try:
                cov[:, :, :, :, i, ii, j, :], rad[:, :, :, :, i, ii, j, :] = ci_hd_lm_cv(y, X, inv_cov[j], k, B, beta_s,
                                                                                         s, t)
            except:
                continue
        ...
        for i3 in np.arange(4):
            for i4 in np.arange(4):
                ...
                pd.DataFrame(np.hstack((np.ones((3 * len(ds) * len(ss), 1)) * l,
                    ...
                    str(l) + '/hd_lm_tp_' + s1 + '_' + s2 + '_cov_' + s3 + '_' + s4 + '.csv', header=False, index=False)
                pd.DataFrame(np.hstack((np.ones((3 * len(ds) * len(ss), 1)) * l,
                                        rad[i1, i2, i3, i4, :, :, :, :].transpose((3, 0, 1, 2)).reshape(
                                            3 * len(ds) * len(ss), len(ks))))).to_csv(
                    str(l) + '/hd_lm_eq_' + s1 + '_' + s2 + '_rad_' + s3 + '_' + s4 + '.csv', header=False, index=False)
